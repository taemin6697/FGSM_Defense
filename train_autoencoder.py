    ...
    for epoch in range(1, n_epochs + 1):
        ...
        with torch.no_grad():
            for data in tqdm(val_dataloader):
                ...
                val_loss1 = MSE(outputs, images.cuda())
                val_loss2 = CrossEntropy(class_, _.cuda())
                val_loss3 = val_loss1 + val_loss2

                val_loss += val_loss3.item() * images.size(0)
                pred = class_.argmax(dim=1)
                pred.to(device)
        ...
        scheduler.step()
        total_val_loss = val_loss / len(val_dataloader.dataset)
